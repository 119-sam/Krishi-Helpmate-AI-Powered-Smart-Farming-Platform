...
const PhotographyTips = () => {
  ...
  const tips = [
    {
      ...
      icon: "📷",
      title: "Clear Images",
      description: "Take clear, well-lit photos of affected plant parts",
    ...
    },
    {
      ...
      description: "Focus on diseased areas for accurate diagnosis",
    ...
    },
    {
      ...
      icon: "☀️",
      title: "Good Lighting",
      description: "Use natural light for best color accuracy",
  ...
  const InteractiveCard = ({ children, tip, className = "" }) => {
    return (
      <div 
        className={`relative group cursor-pointer transition-all duration-500 transform hover:scale-105 hover:-translate-y-1 ${className}`}
    ...
    <section className="relative py-12 overflow-hidden">
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative transform transition-all duration-500 hover:scale-102">
                  ...
                  >
                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center transition-all duration-300 ${
                      ...
                      <span className={`text-lg ${
                        hoveredCard === tip.id ? 'text-white' : 'text-green-600'
                      }`}>
                        {tip.icon}
                      </span>
                    </div>
          ...
          <div className="absolute -bottom-6 -left-6 w-10 h-10 bg-yellow-400/20 rounded-full animate-pulse" style={{animationDelay: '1s'}}></div>
          <div className="absolute top-1/2 right-8 w-6 h-6 bg-blue-400/20 rounded-full animate-pulse" style={{animationDelay: '2s'}}></div>
        </div>
      </div>
